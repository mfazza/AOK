...
- For freeform text, use `type: "text"` and do NOT include an `options` array. When using `type: "choice"`, you MUST provide an `options` array.
- For freeform text, use `type: "text"`. **CRITICAL:** When using `type: "text"`, do NOT include the `options` array parameter in the tool call.
