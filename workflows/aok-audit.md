...
question(
  ...
  options: [
    "{agent-1} ({location})" — {description from frontmatter or first line},
    "{agent-2} ({location})" — {description},
    "{agent-3} ({location})" — {description},
    "All agents" — Audit everything and produce summary comparison table,
    "Something else (I'll point you to it)" — An agent not in these locations
  ]
)
```
